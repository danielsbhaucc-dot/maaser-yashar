...
export function noamChatWelcome(opts: {
  ...
  if (entryCount === 0) {
    return t(
      g,
      `${name}, טוב שחזרת.\nהפנקס ריק — זרוק לי מספרים (משכורת, מסים, צדקה) ואני אציע מה לרשום. בלי דרשות.`,
      `${name}, טוב שחזרת.\nהפנקס ריק — זרקי לי מספרים (משכורת, מסים, צדקה) ואני אציע מה לרשום. בלי דרשות.`
    );
  }
  if (totals.remaining > 0) {
    return t(
      g,
      `${name}, אני כאן.\nנשאר לתת ${money(totals.remaining)} החודש. ספר לי מה חדש — או נחשב יחד מה חסר.`,
      `${name}, אני כאן.\nנשאר לתת ${money(totals.remaining)} החודש. ספרי לי מה חדש — או נחשב יחד מה חסר.`
    );
  }
  if (totals.obligation > 0 && totals.remaining <= 0) {
    return t(
      g,
      `${name}, החודש מכוסה ✨\nרוצה לרשום עוד תנועה, לבדוק חודש אחר, או סתם לוודא שהמספרים מדויקים?`,
      `${name}, החודש מכוסה ✨\nרוצה לרשום עוד תנועה, לבדוק חודש אחר, או סתם לוודא שהמספרים מדויקים?`
    );
  }
  return t(
    g,
    `${name}, אני כאן.\nזרוק לי מספרים — ואני עושה סדר בפנקס. בלי דרשות.`,
    `${name}, אני כאן.\nזרקי לי מספרים — ואני עושה סדר בפנקס. בלי דרשות.`
  );
}
